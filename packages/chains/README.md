...
### BSC Testnet Core Pool override (pre-release / fork)

To generate vTokens for a different BSC Testnet Core Pool (e.g. a pre-release or fork that uses a
different comptroller and VenusLens), set in `.env`:

- `BSC_TESTNET_CORE_LENS` — VenusLens contract address
- `BSC_TESTNET_CORE_COMPTROLLER` — Unitroller/Comptroller contract address

Then run `yarn generate-v-tokens`. When these are set, the script uses them instead of the
addresses from `@venusprotocol/venus-protocol` for the Core Pool, so the generated vToken list
matches what your backend (e.g. venus-api) returns.
